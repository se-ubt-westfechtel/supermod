...
/**
 * UI action for the VCS synchronization operation "Push".
 *
 ...
 * @date	09.11.2015
 */
public class PushHandler extends SuperModCommandHandler {
	...
	protected boolean canRun(List<?> selection) {
		for (IProject p : findRelevantProjects(selection)) {
			try {
				...
				IPushService pushService = getServiceForResource(p, rs,
						IPushService.class);
				if (pushService.canPush(repo)) {
					return true;
				}
			} catch (Exception e) {
				return false;
	...
	protected void run(List<?> selection) {
		...
		if (!notPushed.isEmpty()) {
			warn("Some resources have not been pushed.",
					"The following resources could not be pushed:\n"
					+ linewise(notPushed));
		}
		refresh();
	}
	...
	protected String getTaskDescription() {
		return "Pushing changes to server-side repository";
	}
}
