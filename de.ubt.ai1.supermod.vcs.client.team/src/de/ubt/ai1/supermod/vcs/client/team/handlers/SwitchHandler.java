...
/**
 * UI action for the local VCS operation "Switch". Note that, in the external
 * representation, it has been renamed to "Check-out".
 * 
 * @see ISwitchService
 *
 ...
 * @date	04.08.2014
 */
public class SwitchHandler extends SuperModCommandHandler {
	...
	protected boolean canRun(List<?> selection) {
		for (IProject p : findRelevantProjects(selection)) {
			try {
				...
				ISwitchService switchService = getServiceForResource(p, rs,
						ISwitchService.class);
	...
	protected void run(List<?> selection) {
		List<String> notSwitched = new ArrayList<>();
		for (IProject p : findRelevantProjects(selection)) {
			try {
				ResourceSet rs = createResourceSet();
				String uri = p.getFullPath().toPortableString();
				...
				ISwitchService switchService = getServiceForResource(p, rs,
						ISwitchService.class);
				if (switchService.canSwitch(repo)) {
					if (switchService.doSwitch(repo, rs)) {
						saveAllResources(rs);
					}
					else {
						notSwitched.add(uri);
		...
		if (!notSwitched.isEmpty()) {
			warn("Some resources have not been checked-out.",
					"The following resources could not be checked-out:\n"
					+ linewise(notSwitched));
		}
		refresh();
	}
