 ...
 * one valid feature configuration where it is not included.
 *
 ...
 * @date	14.09.2015
 */
public class ActuallyOptionalFeatureConstraint extends AbstractModelConstraint {
		...
		if (eo instanceof Feature) {
			...
			if (f.getMandatory() == null && f.getIncomingRoots().isEmpty()) {
				...
				boolean sat = fm.validate(ob) != Tristate.FALSE 
						&& fm.isSatisfiable(ob);
				if (sat) {
					ob.bind(f.getFeatureOption(), Tristate.FALSE);
					ob = fm.applyCompletions(ob);
